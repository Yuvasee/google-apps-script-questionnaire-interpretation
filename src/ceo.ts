...
export function processResponses() {
	...
	initResultsSheets();
	renderResults(companies, participantResponses);
}

function renderResults(companies: string[], participantResponses: ParticipantResponse[]) {
	const participantQuestionnaires = participantResponses
	...
	questionnaires.forEach(q => renderQuestionnaire(companies, q, participantQuestionnaires))
}
...
function renderQuestionnaire(
	...
	const companySheet = spreadsheet.getSheetByName('Results');
	const startRow = 1 + (participantResponses.length + 2) * questionnaire.index;
...
function initResultsSheets() {
	...
	if (!sheetNames.includes(resultsSheetName)) {
		const newSheet = spreadsheet.insertSheet();
		newSheet.setName(resultsSheetName);
	} else {
		const companySheet = spreadsheet.getSheetByName(resultsSheetName);
		companySheet.clear();
	}
}
...
function getQuestionnaire1(): Questionnaire {
	const questionnaire = {
		...
		questions: [
			...
			{ index: 22, title: 'Меня больше раздражают люди которые:', options: { A: 'своими грубыми шутками вгоняют людей в краску', B: 'затрудняюсь ответить', C: 'создают неудобства для меня, опаздывая на условленную встречу' } },
			...
			{ index: 67, title: 'Если очень трудный вопрос требует от меня очень много усилий, то я:', options: { A: 'заняться другим вопросом', B: 'затрудняюсь ответить', C: 'еще раз попытаться решить этот вопрос' } },
			...
			{ index: 77, title: 'Если бы я мог перенестись в прошлое, то больше я хотел бы встретиться:', options: { A: 'колумбом', B: 'не знаю', C: 'пушкиным' } },
